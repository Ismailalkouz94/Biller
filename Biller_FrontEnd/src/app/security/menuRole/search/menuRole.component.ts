...
roleGroupId;
  ngOnInit() {
    this.roleGroupId=localStorage.getItem("roleGroupId");
    ...
    this.getMenuRolesDataByRoleGroupId(this.roleGroupId);
//     if(this.roleGroupId !== "" &&
//     this.roleGroupId !== "undefined" &&
...
//     }
  ...
  getMenuRolesData() {
   ...
   getMenuRolesDataByRoleGroupId(roleGroupId) {


    let headers = new Headers();
    headers.append("Content-Type", "application/json");
    let options = new RequestOptions({ headers: headers, params: { roleGroupId: roleGroupId } });
    this.http.get(appConfig.apiUrl + 'menuRole/findByRoleGroupId', options).map(res => res.json()).subscribe((data) => {
      this.menuRolesList = data;
    })
  }
  getRoleGroupsData() {
